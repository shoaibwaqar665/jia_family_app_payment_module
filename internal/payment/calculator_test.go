...
func TestCalculator_Apply_BasicRules(t *testing.T) {
	...
	now := time.Now()
	startTime := now.Add(-30 * time.Minute) // Start 30 minutes ago
	endTime := now.Add(30 * time.Minute)    // End 30 minutes from now

	_ = store.Upsert(PricingRule{ID: "loc-us", Type: RuleTypeLocation, Location: "US", Factor: 1.10, Enabled: true})
	_ = store.Upsert(PricingRule{ID: "happy-hour", Type: RuleTypeTime, StartTime: &startTime, EndTime: &endTime, Factor: 0.90, Enabled: true})
...
func TestCalculator_Discount_ClampedAt50(t *testing.T) {
	...
	_ = store.Upsert(PricingRule{ID: "deep-discount", Type: RuleTypeTime, StartTime: &startTime, EndTime: &endTime, Factor: 0.2, Enabled: true})
	calc := NewCalculator(store)
